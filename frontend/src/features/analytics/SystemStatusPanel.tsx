    ...
    <div className="flex items-center gap-2 text-sm">
      ...
      <span className="ml-auto flex items-center gap-1.5">
        <span
          ...
          aria-hidden="true"
        />
        <span className={`font-mono text-mono-micro lowercase ${isUp ? "text-ink-muted" : "font-medium text-severity-p0"}`}>
          {isUp ? "up" : "down"}
        </span>
        <span className="w-14 text-right font-mono text-mono-num tabular-nums text-ink-muted">{latencyMs}ms</span>
      </span>
    </div>
  );
}
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-baseline justify-between">
        ...
        <span className="font-mono text-mono-num font-medium tabular-nums text-ink">
          {pct}% <span className="text-ink-muted">· {depth}/{capacity}</span>
        </span>
      </div>
      <div className="h-2 w-full overflow-hidden rounded-full bg-border" role="meter" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} aria-label="Ingestion buffer fill">
      ...
      <div className="flex items-center gap-1.5 text-xs">
        ...
        <span className={`font-body text-prose ${shedding ? "font-medium text-severity-p0" : "text-ink-muted"}`}>
    ...
    <Card className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="font-body text-prose text-ink-muted">Live backpressure &amp; dependency health · updates every 5s</p>
        </div>

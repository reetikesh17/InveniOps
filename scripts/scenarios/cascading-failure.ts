...
import { ApiClient, type ComponentType, type Severity, type SignalInput } from "./helpers/apiClient.js";
...
import { banner, section, ok, fail, info, note, fmtDuration, fmtElapsed } from "./helpers/format.js";
...
function parseArgs(argv: readonly string[]): Args {
  ...
  return {
    ...
    databaseUrl:
      get("--database-url", process.env["DATABASE_URL"] ?? "postgresql://ims_user:ims_password@localhost:5432/ims"),
    containerName: get("--container", process.env["BACKEND_CONTAINER_NAME"] ?? "inveniops-ims-backend-1"),
...
function rampRatesPerSecond(fromPerSecond: number, toPerSecond: number, durationSeconds: number): readonly number[] {
...
async function postBatch(api: ApiClient, signals: readonly SignalInput[], totals: RunTotals): Promise<void> {
  if (signals.length === 0) {
    return;
  }
  ...
  if (result.status !== 202) {
    fail(`unexpected status ${result.status} posting ${signals.length} signal(s) for a batch — see response above`);
  }
}
...
async function runSignalSpec(
  ...
  info(`${spec.componentId}: ramping ${from}/s → ${to}/s over ${duration}s, reported severity ${spec.severity}`);
...
function collectComponents(scenario: Scenario): Map<string, { componentType: ComponentType; role: "baseline" | "incident" }> {
  const components = new Map<string, { componentType: ComponentType; role: "baseline" | "incident" }>();
...
async function main(): Promise<void> {
  ...
  note(`API: ${args.apiUrl}  |  speed: ${args.speed}x  |  real-time total: ${fmtDuration(180)}, this run: ${fmtDuration(180 / args.speed)}`);
  ...
  if (preflight.body.status !== "healthy") {
    fail(`backend reports "${preflight.body.status}", not "healthy" — dependencies: ${JSON.stringify(preflight.body.dependencies)}`);
  ...
  note(`recording baseline signal counts for ${components.size} components before sending anything (makes this script safely re-runnable)`);
  ...
  ).catch((error: unknown) => note(`queue didn't fully report empty in time (${String(error)}) — continuing to verify anyway`));

  banner("Verifying outcomes");

  section("Buffer absorbed the burst without loss");
  console.log(`  ${totals.sent} signals sent across the whole scenario, ${totals.accepted} accepted, ${totals.dropped} dropped by the buffer, ${totals.rateLimitRetries} rate-limit retries.`);
  if (totals.dropped === 0) {
    ok("zero signals dropped by the ingestion buffer — no loss");
  } else {
    fail(`${totals.dropped} signal(s) were dropped (503 buffer_saturated) — see per-beat log above`);
  }
  ...
  for (const [componentId, meta] of components) {
    ...
    try {
      await waitFor(
        ...
        { timeoutMs: 30_000, intervalMs: 1_000, description: `${componentId}'s work item to reach signalCount ${expected}` },
      );
    } catch (error) {
      fail(String(error));
      continue;
    ...
    if (matchedExactly) {
      ...
      note(`${line} — note: exceeds this run's expected count, other traffic may be hitting the same component`);
    }
    verified.push({ componentId, componentType: meta.componentType, role: meta.role, workItemId: row.id, severity: row.severity as Severity, signalCount: row.signalCount });
  }

  const rdbmsComponent = verified.find((c) => c.componentId === "DB_PRIMARY_01");
  if (rdbmsComponent) {
    ok(`debouncing: ${perComponent.get("DB_PRIMARY_01")?.sent ?? 0} RDBMS signals collapsed into exactly 1 work item (${rdbmsComponent.workItemId}) — enforced by the Postgres partial unique index, not just the Redis fast path`);
  }
  ...
  for (const component of incidentComponents) {
    ...
    const expectedAlertSeverity = reportedSeverity && rank(reportedSeverity) < rank(expectedFloor) ? reportedSeverity : expectedFloor;

    try {
      const logs = await waitForValue(
        () => findAlertLogsSince(args.containerName, scenarioStartedAtIso, component.componentId),
        (lines) => lines.length > 0,
        { timeoutMs: 20_000, intervalMs: 1_000, description: `an ALERT log line for ${component.componentId}` },
      );
      const firstAlert = logs[0]!;
      const matches = firstAlert.severity === expectedAlertSeverity;
      const reconciledNote = reportedSeverity !== expectedAlertSeverity ? ` (reported ${reportedSeverity}, floor ${expectedFloor} — floor won)` : ` (matches floor ${expectedFloor})`;
      if (matches) {
        ok(`${component.componentId} (${component.componentType}): alert dispatched at ${firstAlert.severity}${reconciledNote}`);
      } else {
        fail(`${component.componentId}: expected alert severity ${expectedAlertSeverity}, got ${firstAlert.severity}`);
      }
    } catch (error) {
      fail(`${component.componentId}: no ALERT log line found — ${String(error)} (is BACKEND_CONTAINER_NAME correct? currently "${args.containerName}")`);
    }
  }
  ...
  note(`next: npm run replay-lifecycle   (walks the ${incidentComponents.length} incident work items to CLOSED with a real RCA and MTTR)`);

...
import { CONTAINERS, pause, unpause, isRunning, statsSnapshot, ensureRunning } from "./helpers/docker.js";
...
describe("chaos: slow persistence (Mongo paused)", () => {
  ...
  it(
    "keeps accepting signals fast while Mongo is paused, stays alive and bounded, and drains fully once unpaused",
    async () => {
      ...
      try {
        ...
        try {
          await waitFor(
            ...
            { timeoutMs: 60_000, intervalMs: 1_000, description: "all 40 accepted signals to persist after Mongo recovers" },
          );

          const persistedIds = new Set(
            (await db.collection("signals").find({ signalId: { $regex: `^${RUN_TAG}-` } }, { projection: { signalId: 1 } }).toArray()).map(
              (doc) => doc["signalId"] as string,
            ),
          );

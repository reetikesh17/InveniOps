...
import { getHealth, postSignals, transitionIncident, type SignalInput } from "./helpers/apiClient.js";
import { makePrismaClient } from "./helpers/dataClients.js";
import type { PrismaClient } from "@prisma/client";

const RUN_TAG = `chaos-pgoutage-${Date.now()}`;

async function waitForWorkItemByComponent(prisma: PrismaClient, componentId: string, timeoutMs = 30_000): Promise<string> {
  let foundId: string | null = null;
  await waitFor(
    async () => {
      const workItem = await prisma.workItem.findFirst({ where: { componentId }, orderBy: { createdAt: "desc" } });
      if (workItem) {
        foundId = workItem.id;
        return true;
      }
    ...
    { timeoutMs, intervalMs: 500, description: `a work item to be created for component ${componentId}` },
  );
  return foundId!;
}
...
describe("chaos: Postgres outage", () => {
  ...
  it(
    "retries visibly, reports 503 naming postgres, leaves state uncorrupted, and recovers without intervention",
    async () => {
      ...
      try {
        ...
        const failedTransition = await transitionIncident(workItemId, "INVESTIGATING", "chaos-test", 20_000);
      ...
      const recoveredTransition = await transitionIncident(workItemId, "INVESTIGATING", "chaos-test", 10_000);

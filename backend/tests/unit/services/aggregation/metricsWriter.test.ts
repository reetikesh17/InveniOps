...
} from "../../../../src/services/aggregation/metricsWriter.js";
import type {
  AlertDispatchPoint,
  MttrPoint,
  SignalVolumePoint,
...
function workingRepo(): {
  ...
  readonly recordSignalVolume: ReturnType<
    typeof vi.fn<(points: readonly SignalVolumePoint[]) => Promise<void>>
  >;
  readonly recordWorkItemsCreated: ReturnType<
    typeof vi.fn<(points: readonly WorkItemCreatedPoint[]) => Promise<void>>
  >;
  readonly recordStateTransitions: ReturnType<
    typeof vi.fn<(points: readonly StateTransitionPoint[]) => Promise<void>>
  >;
  readonly recordMttr: ReturnType<typeof vi.fn<(points: readonly MttrPoint[]) => Promise<void>>>;
  readonly recordAlertDispatches: ReturnType<
    typeof vi.fn<(points: readonly AlertDispatchPoint[]) => Promise<void>>
  >;
} {
  const recordSignalVolume = vi
    .fn<(points: readonly SignalVolumePoint[]) => Promise<void>>()
  ...
  const recordStateTransitions = vi
    ...
    .mockResolvedValue(undefined);
  const recordMttr = vi
    .fn<(points: readonly MttrPoint[]) => Promise<void>>()
    .mockResolvedValue(undefined);
...
describe("MetricsWriter", () => {
  it("delegates every method to the underlying repository when writes succeed", async () => {
    ...
    await writer.recordSignalVolume([
      { ts: new Date(), componentId: "c", severity: "P1", count: 1 },
    ]);
    await writer.recordWorkItemsCreated([
      { ts: new Date(), componentType: "CACHE", severity: "P1" },
    ]);
    await writer.recordStateTransitions([
      { ts: new Date(), fromState: "OPEN", toState: "INVESTIGATING", timeInStateMs: 10 },
    ]);
    await writer.recordMttr([
      { ts: new Date(), componentType: "CACHE", severity: "P1", componentId: "c", mttrMs: 1000 },
    ]);
    await writer.recordAlertDispatches([
      { ts: new Date(), channel: "slack", outcome: "delivered" },
    ]);
  ...
  it("recordSignalVolume: a repository failure is logged and swallowed, never rejected, and not retried", async () => {
    ...
    await expect(
      writer.recordSignalVolume([{ ts: new Date(), componentId: "c", severity: "P1", count: 1 }]),
  ...
  it("recordWorkItemsCreated: a repository failure is logged and swallowed", async () => {
    ...
    await expect(
      writer.recordWorkItemsCreated([{ ts: new Date(), componentType: "CACHE", severity: "P1" }]),
    ).resolves.toBeUndefined();
